      ...
      <div className="mx-auto max-w-3xl px-6 py-12 sm:py-16">
        <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm sm:p-8">
          <div className="flex items-start gap-5">
            {provider.avatar_url ? (
              <img src={provider.avatar_url} alt={provider.full_name} className="h-16 w-16 rounded-full object-cover" />
            ) : (
            ...
            <div>
              ...
              <div className="mt-1 flex flex-wrap items-center gap-3 text-sm text-gray-500">
                <span className="rounded-full bg-fairway-50 px-3 py-0.5 font-medium text-fairway-700">{roleLabel}</span>
                {provider.hourly_rate && <span>${Number(provider.hourly_rate).toFixed(0)}/hr</span>}
                {provider.years_experience && <span>{provider.years_experience} years exp.</span>}
                ...
                <div key={service.id} className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <p className="mt-2 text-xs text-gray-400">{service.duration_minutes} minutes</p>
                    </div>
                    <div className="flex flex-col items-end gap-2">
                      <p className="text-xl font-bold text-fairway-700">${Number(service.price).toFixed(0)}</p>
                      <Link
        ...
        <div className="mt-8 text-center">
          <Link href="/browse" className="text-sm text-gray-400 hover:text-fairway-600">← Browse all providers</Link>
        </div>
      </div>
    </div>
  );
