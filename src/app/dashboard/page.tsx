  ...
  return (
    <div className="min-h-[calc(100vh-65px)] bg-brand-cream">
      <div className="mx-auto max-w-4xl px-6 py-12">
        ...
        {isCaddie && (
          <div className="mt-8">
            <h2 className="font-display text-lg font-bold text-brand-charcoal">Subscription</h2>
            <div className="mt-4">
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-3">
          <div className="rounded-xl border border-brand-border bg-white p-5 shadow-sm">
            <p className="text-sm font-medium text-brand-muted">Total Earned</p>
            <p className="mt-1 text-2xl font-bold text-brand-green-700">${totalEarnings.toFixed(0)}</p>
            <p className="mt-1 text-xs text-brand-muted">{completedBookings.length} completed</p>
          </div>
          <div className="rounded-xl border border-brand-border bg-white p-5 shadow-sm">
            <p className="text-sm font-medium text-brand-muted">Upcoming</p>
            <p className="mt-1 text-2xl font-bold text-brand-charcoal">${pendingEarnings.toFixed(0)}</p>
            <p className="mt-1 text-xs text-brand-muted">{upcomingBookings.length} confirmed</p>
          </div>
          <div className="rounded-xl border border-brand-border bg-white p-5 shadow-sm">
            <p className="text-sm font-medium text-brand-muted">Pending Requests</p>
            <p className="mt-1 text-2xl font-bold text-yellow-600">{pendingBookings.length}</p>
            <p className="mt-1 text-xs text-brand-muted">awaiting your response</p>
          </div>
        </div>
        ...
        {pendingBookings.length > 0 && (
          <div className="mt-8">
            <h2 className="flex items-center gap-2 text-lg font-bold text-brand-charcoal">
              ...
              {pendingBookings.map((booking) => {
                ...
                return (
                  <div key={booking.id} className="rounded-xl border-2 border-yellow-200 bg-white p-5 shadow-sm">
                    <div className="flex items-start justify-between">
                      <div>
                        <h3 className="font-semibold text-brand-charcoal">${Number(booking.total_price).toFixed(0)}</h3>
                        <p className="mt-1 text-sm text-brand-muted">
                          Requested by <span className="font-medium text-brand-charcoal">{player?.full_name || "Player"}</span>
                        </p>
                        <p className="mt-1 text-sm text-brand-muted">
                    ...
                    {booking.notes && <p className="mt-3 rounded-lg bg-brand-cream p-3 text-sm text-brand-muted">&quot;{booking.notes}&quot;</p>}
                    <div className="mt-4">
        ...
        {upcomingBookings.length > 0 && (
          <div className="mt-8">
            <h2 className="font-display text-lg font-bold text-brand-charcoal">Upcoming Bookings</h2>
            <div className="mt-4 space-y-4">
        ...
        <div className="mt-8 rounded-2xl border border-brand-border bg-white p-6 shadow-sm sm:p-8">
          <div className="flex items-center justify-between">
            <h2 className="font-display text-lg font-bold text-brand-charcoal">Your Profile</h2>
        ...
        {isCaddie && (
          <div className="mt-8">
            <h2 className="font-display text-lg font-bold text-brand-charcoal">Receive Payments</h2>
            <div className="mt-4">
