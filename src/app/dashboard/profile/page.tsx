...
export default async function DashboardProfilePage() {
  ...
  if (isCaddie) redirect("/caddie/profile");

  const table = isCaddie ? "caddies" : "instructors";
  ...
  let reviews: (ReviewRow & { reviewer_name: string })[] = [];
  if (isCaddie) {
  ...
  if (builderApiKey) {
    try {
      builderContent = await fetchOneEntry({
        ...
        userAttributes: { urlPath: "/dashboard/profile" },
      });
    } catch {
  ...
  if (builderApiKey && builderContent) {
    return (
      <div className="min-h-[calc(100vh-65px)]">
        ...
        {/* Edit profile and contact stay in code so caddies can always edit */}
            ...
            <div className="p-6 sm:p-8">
              <div className="flex flex-col items-center gap-5 sm:flex-row sm:items-start">
                ...
                <div className="relative shrink-0">
                  ...
                  {row.verified && (
                    <div
                      className="absolute -bottom-0.5 -right-0.5 flex h-8 w-8 items-center justify-center rounded-full border-2 border-white bg-brand-green-500 shadow-sm"
                      title="Verified"
                    >
                ...
                <div className="min-w-0 flex-1 text-center sm:text-left">
                  ...
                  {locationLine && (
                    <p className="mt-1 text-base text-brand-muted">{locationLine}</p>
                  )}
                  <div className="mt-4 flex flex-wrap items-center justify-center gap-3 sm:justify-start">
                    <span className="inline-flex items-center rounded-full bg-brand-green-100 px-3.5 py-1 text-sm font-medium text-brand-green-800">
                      {roleLabel}
                    </span>
                    {isCaddie && row.subscription_status === "active" && (
                      <span className="inline-flex items-center rounded-full bg-brand-green-50 px-3.5 py-1 text-sm font-medium text-brand-green-700">
                        Subscribed
                      </span>
                    )}
                    {isCaddie && row.subscription_status !== "active" && (
                      <span className="inline-flex items-center rounded-full bg-brand-muted/15 px-3.5 py-1 text-sm font-medium text-brand-muted">
                        Not subscribed
                      </span>
                    )}
                    {isCaddie && reviewCount > 0 && averageRating != null && (
        ...
        <div className="mt-8 grid gap-8 pb-16 lg:grid-cols-[1fr,320px] lg:items-start">
          ...
          <div className="space-y-8">
            ...
            <section className="rounded-2xl border border-brand-border bg-white p-6 shadow-sm sm:p-8">
              ...
              {row.bio ? (
                <p className="mt-3 whitespace-pre-wrap text-[15px] leading-relaxed text-brand-charcoal/90">
                  {row.bio}
                </p>
              ) : (
                <p className="mt-3 text-[15px] text-brand-muted">
                  Add a few lines about yourself in Edit profile so golfers can get to know you.
                </p>
              )}
            </section>
            ...
            <section className="rounded-2xl border border-brand-border bg-white p-6 shadow-sm sm:p-8">
              ...
              <p className="mt-3 text-[15px] text-brand-charcoal/90">
                {row.years_experience != null
                  ? `${row.years_experience} ${row.years_experience === 1 ? "year" : "years"} of experience`
                  : "—"}
              </p>
            </section>
            ...
            {isCaddie && (
              <section className="rounded-2xl border border-brand-border bg-white p-6 shadow-sm sm:p-8">
                ...
                {reviewCount === 0 ? (
                  <p className="mt-3 text-[15px] text-brand-muted">
                    No reviews yet. Complete bookings to build your reputation with golfers.
                  </p>
                ) : (
                  <>
                    <div className="mt-4 flex items-baseline gap-3">
                      ...
                      <span className="text-[15px] text-brand-muted">
                    ...
                    <ul className="mt-6 space-y-5">
                      {reviews.map((r) => (
                        ...
                        >
                            ...
                            <div className="flex items-center gap-1.5">
                              <span className="flex">
                                {[1, 2, 3, 4, 5].map((i) => (
                                  <svg
                                    key={i}
                                    className={`h-4 w-4 ${i <= r.rating ? "text-brand-gold-500" : "text-brand-border"}`}
                                    fill="currentColor"
                                    viewBox="0 0 20 20"
                                  >
                                    <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                          ...
                          {r.comment && (
                            <p className="mt-2 text-[15px] leading-relaxed text-brand-charcoal/85">
                              {r.comment}
                            </p>
                          )}
                        </li>
          ...
          {/* Right: Contact + View profile + Edit */}
          <div className="space-y-6 lg:sticky lg:top-24">
            <div className="rounded-2xl border border-brand-border bg-white p-6 shadow-sm">
              ...
              <Link
                ...
                className="mt-5 flex w-full items-center justify-center rounded-xl bg-brand-green-600 px-4 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-brand-green-700"
              >
                View public profile
              </Link>
            </div>
